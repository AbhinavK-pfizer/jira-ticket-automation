...
async def upload_transcript(transcriptText: str = Form(None), transcriptFile: UploadFile = None):
    token = generate_token(verification_url, client_id, client_secret)
    if not token.startswith("Bearer"):
        print("bearer problem")
    ...
    if transcriptText:
        prompt = base_prompt + transcriptText
        print("prompt created")
    elif transcriptFile:
        try:
            file_content = await transcriptFile.read()
    ...
    response = requests.request("POST", mulesoft_url, headers=headers, data=payload)

    if response.status_code == 200:
        print("YO WE GOT SMTH")
        result = response.json()["result"]
        print(result)
        return result
    else:
        print("unable to get a response with status code {}".format(response.status_code))

    #print(f"Payload to VOX API: {payload}")
    #print(f"Headers to VOX API: {headers}")
    #print(f"Received text: {transcriptText}")